...
def insert(mol2, target=None, join=True, p2b=True, 
	...
	if alpha or alpha2 or alpha3 or dihedral:
		...
		AddAttr.addAttributes(mol2.replace('.mol2','.attr'), raiseTool=False)
		for a in tmpl.atoms:
			if a.element.name == 'H':
				tmpl.deleteAtom(a)
				continue
...
def library(cbase, linkers, fragments, dihedral=None, alpha=None):
	mol = cbase.atoms[0].molecule
	explore = itertools.product(range(len(linkers)), range(len(fragments)))
	library = {}
	for i, j in explore:
		#TODO: Create custom molecule_from_atoms()
		new = SplitMolecule.split.molecule_from_atoms(mol, cbase.atoms)
		target = new.atoms[-1]
		linker = insert(linkers[i], target=target, alpha=alpha, alpha2=120.0)
		linker_anchor = [ a for a in linker if a.anchor in (4,6,8)][0]
		insert(fragments[j], target=linker_anchor, alpha2=114.125)
