...
# Python
import random, numpy, deap, sys, math, itertools, logging
...
log = gaudi.parse.enable_logging(cfg.default.outputpath)
...
deap.creator.create("Individual", dict, fitness=deap.creator.FitnessMax,
					fitness_names=['{}_{}'.format(*obj) 
								for obj in enumerate(cfg.allobjectives)])
...
if __name__ == "__main__":	
	...
	rank = gaudi.box.write_individuals(hof, cfg.default.outputpath,	
								cfg.default.name, evaluate)
	out = open(cfg.default.outputpath+cfg.default.name+'.gaudi', 'w+')
	...
	if cfg.ga.history:
		...
		# plt.show()
		# plt.savefig(cfg.default.outputpath + 'history.png')
		# Dump log
		import pickle
		cp = dict(pop=pop, gens=cfg.ga.gens, log=log, history=history,
				rndstate=random.getstate())
		with open(cfg.default.outputpath + 'dump_log.pkl', "wb") as cp_file:
