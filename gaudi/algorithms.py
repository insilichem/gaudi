...
def ea_mu_plus_lambda(population, toolbox, mu, lambda_, cxpb, mutpb, ngen, cfg,
    ...
    for gen in xrange(1, ngen + 1):
        try:
            ...
            population_ = population[:]
            if halloffame and gen % cfg.output.check_every:
                dump_population(halloffame, cfg)
    return population_, logbook

def dump_population(population, cfg, checkpoint=False):
    ...
    if checkpoint:
        try:
            os.makedir(os.path.join(cfg.output.path, 'check'))
        except:
            pass
        else:
            gaudi_output = os.path.join(cfg.output.path, 'check', cfg.output.name + '.gaudi-output')
